...
1. **`tenants` no declaraba conexión.** `App\Models\Tenant` extiende `Stancl\Tenancy\...\Tenant`, que resuelve su conexión leyendo `config('tenancy.database.central_connection')`. La migración original creaba la tabla en la conexión por defecto; "funcionaba" solo por coincidencia (mismo archivo SQLite). Se corrigió la migración para declarar explícitamente `protected $connection = 'central'`.
2. **`tenancy.database.central_connection` nunca se había publicado/fijado.** Este es el hallazgo más importante de los tres: el paquete `stancl/tenancy` nunca tuvo su archivo de configuración publicado en el proyecto, así que ese valor caía en el default del propio paquete, `env('DB_CONNECTION', 'central')` — es decir, **la misma conexión que usa el resto de la app** (HOSPITAL), no una conexión `central` dedicada. En SQLite de un solo archivo esto era invisible (todo vive en el mismo archivo físico); se volvió un error real (`SQLSTATE[HY000]: no such table: tenants`) en cuanto `central` pasó a ser una conexión con datos propios y separables (como ocurre en las pruebas, con dos SQLite `:memory:` independientes, y como ocurriría en producción con PostgreSQL real). Se corrigió publicando `config/tenancy.php` y fijando `'database.central_connection' => 'central'` explícitamente, para que `Tenant` (y cualquier código que dependa de "la conexión central" de Stancl) apunte de verdad a la conexión `central` de `config/database.php`, sin importar el valor de `DB_CONNECTION`.
3. **FK física de `users.tenant_id` hacia `tenants.id`.** Con `tenants` en `central` y `users` en `default`, esa FK sería una FK entre bases — imposible en PostgreSQL real y prohibida por esta arquitectura. Se retiró la restricción de FK (se conserva el índice y el `unique(tenant_id, email)`); `User::tenant()` sigue funcionando porque es una relación Eloquent por valor de columna, no por integridad referencial del motor.
4. **Unicidad de `patients.code`.** Era única a nivel global; RF-11 exige un correlativo único **por hospital** (`PAC-0001` en dos hospitales distintos es válido). Se cambió a `unique(tenant_id, code)`.

> Ninguno de estos cuatro cambios altera el comportamiento observable de otro módulo: no dependen de en qué conexión física vive `tenants`/`users`, solo de que los modelos `Tenant`/`User` sigan funcionando igual (y siguen igual; de hecho, `Tests\Feature\Modulo20\EloquentLabResultRepositoryIntegrationTest`, de otro compañero, pasó a estar en verde tras este fix porque también usaba `Tenant::factory()`). Según `docs/module-03/vista-arquitectonica.md`, ningún módulo depende de estas restricciones específicas, solo de las relaciones `patient_id`/`tenant_id`.
